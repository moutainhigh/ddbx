...
public class DbTools {
    ...
    public List<Map<String, String>> reclist(String sql) {
        ...
        try {
            ...
            for (int i = 1; i <= columnCount; i++) {
                lsFields.add(md.getColumnLabel(i));
            }
    ...
    public Map<String, String> recinfo(String sql, boolean field, String retype, Connection conf) {
        //function recinfo($sql,$field=false,$retype='auto',$conf=null){
        ...
        try {
            ...
            while (rs.next()) {
                for (int i = 1; i <= columnCount; i++) {
                    ...
                    String vString = paramvalue(rs, md, i);
                    result.put(md.getColumnLabel(i), vString);
                }
                break;
            }
            rs.close();
...
/*        try {
            if (Tools.myisnull(vString) == false && md.getColumnTypeName(i) == "DATETIME") {//去掉DATE-TIME的.0
