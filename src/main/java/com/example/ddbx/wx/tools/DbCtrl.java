...
import com.example.ddbx.tt.tool.DbConfig;
 ...
 */
public class DbCtrl {
    private String ver = "1.1 build 1206";
...
//    }
    ...
    private void initconn() {
        if (this.conn != null) {
            this.myDbTools = new DbTools(conn, this.showlog);
    ...
    public List<Map<String, String>> lists(String wheres, String f) {
        ...
        sql = "select SQL_CALC_FOUND_ROWS " + (Tools.myisnull(f) == false ? f : this.fields) + " from " + this.table
                + " t " + this.leftsql + " " + this.rightsql + " " + this.innersql + " " + (Tools.myisnull(wheres) == false ? " where " + strWhere : (Tools.myisnull(strWhere) ? "" : " where " + strWhere)) + getListSqlStrlimit() + " " + this.groupby + " " + this.orders;
    ...
    public Map<String, String> info(long id, String f) {
        ...
        /*
         * todo ,left join，right join，inner join查询以及groupby查询功能的加入 if
    ...
    public String getPage(String style, int l, boolean auto_hide) {
        ...
        return strGetPage(this.pages, this.p, style, l, auto_hide);
    }
    ...
    public void closeConn() {
        ...
        this.closenow++;
        mylog("累计关闭conn.close()连接数：" + this.closenow);
        try {
