...
public class AdminAgp extends DbCtrl {
  ...
  private TtMap infoAgp = info(Tools.strToLong(minfo.get("agp_id")));// 用户所属角色组信息
  ...
  public String getAgpSqlWhere() { // 根据当前登陆用户获取可以操作的模块的sql的where
    ...
    if (!Tools.isSuperAdmin(minfo)) {// 如果不是内部超级管理员
      ...
      String purview_map = getAgpStrFs("fs", minfo.get("fsid")); // 所在公司的权限
      sql = sql + " AND t.id in (" + purview_map + ")";
      String agp_id = minfo.get("agp_id");
      purview_map = getAgpStrFs("admin_agp", agp_id); // 所属角色权限
      sql = sql + " AND t.id in (" + purview_map + ")";
    } else {
      sql = "";
    }
  ...
  private TtMap infoModal(String modalId) {
    return Tools.recinfo("select * from sys_modal where id='" + modalId + "'");
  }
  ...
  public void doGetList(HttpServletRequest request, TtMap post) {
    ...
    if (Tools.isSuperAdmin(minfo)) {
      ...
      tmpWhere = " AND fsid=" + minfo.get("fsid") + " AND systag=0";
    }
    ...
    for (TtMap tmpInfo : list) { // 获取当前角色组的成员列表
      ...
      if (Tools.isSuperAdmin(minfo)) {
        tmpInfo.put("fsname", Tools.unDic("fs", Tools.strToLong(tmpInfo.get("fsid"))));
