...
public class hbyh_zxlr extends DbCtrl {
    private final String title = "征信录入";
    private String orderString = "ORDER BY t.dt_edit DESC"; // 默认排序
    private boolean canDel = false;
    private boolean canAdd = false;
    private final String classAgpId = "177"; // 随便填的，正式使用时应该跟model里此模块的ID相对应
    ...
    public void doGetList(HttpServletRequest request, TtMap post) {
        ...
        String fieldsString = "t.*" +
                ",f.name as fsname" +
                ",a.name as adminname" +
                ",hbyh_dygd.id as dy_id" +
                ",hbyh_dygd.bc_status as dy_bc_status"; /*+
...
//            whereString += " AND ("; // 显示自己和下级公司的
        ...
        leftsql = " LEFT JOIN fs f ON f.id=t.gems_fs_id " +
                ...
                " LEFT JOIN hbyh_dygd hbyh_dygd ON hbyh_dygd.icbc_id=t.id";
