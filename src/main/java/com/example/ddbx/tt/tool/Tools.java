...
public class Tools<K, V> {
    ...
    public static String dicopt(String tbName, long id) {
        ...
        dbt.nopage = true;
        try {
            List<Map<String, String>> lttmp = dbt.lists("", "t.name,t.id");
    ...
    public static String dicopt(String tbName, long id, String wheres, String re) {
        ...
        try {
            mylog(wheres);
            List<Map<String, String>> lttmp = dbt.lists(wheres, "t.name,t.id");
            ...
            for (int i = 0; i < lttmp.size(); i++) {
                ...
                if (re.equals("json")) {
                    ...
                    result = result + "<option value=\"" + lttmp.get(i).get("id") + "\""
                            + (nowid == id ? " selected" : "") + ">" + lttmp.get(i).get("name") + "</option>";
                }
            }
            if (re.equals("json")) {
     ...
     */
    public static String dicopt(String tbName, long id, Map<String, String> post, String re) {
        DbCtrl dbt = new DbCtrl(tbName);
        dbt.showall = true;
        dbt.getall = true;
        dbt.nopage = true;
    ...
    public static void setNowUser(long id, boolean isadmin) {
        ...
        session.setAttribute("tt_mid", id);
        session.setAttribute("idmd5", idmd5);
        ...
        recexec("insert into sys_session set idmd5='" + idmd5 + "', mid='" + id + "',logdt='" + time() + "',logip='"
                + sip + "'");
    }
     ...
     * @return:
     */
    public static Map<String, String> minfo(String tbName) {
        return Tools.recinfo("select * from " + tbName + " where id=" + mid());
    }

    public static Map<String, String> minfo() {
        return Tools.recinfo("select * from " + Config.DB_USERTABLENAME + " where id=" + mid());
    }
}
